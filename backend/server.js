...
// Agregar después de la conexión a MongoDB
const initializeProyectores = async () => {
  try {
    const count = await Proyector.countDocuments();
    if (count === 0) {
      await Proyector.create([
        {
          nombre: 'Proyector 1',
          codigo: 'PRY001',
          estado: 'devuelto',
        ...
        },
        {
          ...
          estado: 'en uso',
          ubicacion: 'Sala 202',
          observaciones: 'Cable HDMI dañado'
        }
      ]);
      console.log('Proyectores de prueba creados');
    }
  } catch (error) {
    console.error('Error al crear proyectores de prueba:', error);
  }
};
