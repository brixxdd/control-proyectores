  ...
  const verifyToken = async (req, res, next) => {
    ...
    // Permitir acceso a nuevos usuarios que no tengan token
    if (!token) {
        // Permitir que continúe la solicitud si no hay token
        return next();
    }
    ...
    try {
        ...
        console.log("Payload de Google:", payload); // Verifica el payload
        ...
        console.error("Error al verificar el token de Google:", error); // Log de error
        ...
        try {
            ...
            console.error("Error al verificar el token JWT:", error); // Log de error
  ...
  app.get('/check-session', verifyToken, async (req, res) => {
    const email = req.user ? req.user.email : null; // Obtener el email del usuario autenticado
    ...
    // Si no hay usuario autenticado, permitimos el acceso
...
app.post('/login', async (req, res) => {
  ...
  try {
      ...
      // Permitir también al administrador por su correo
